module God
  module Conditions
    ...
    # Condition Symbol :memory_usage_with_children
    # Type: Poll
    #
    # Trigger when the resident memory of a process and its children is
    ...
    # memory (from a Watch):
    #
    #   on.condition(:memory_usage_with_children) do |c|
    ...
    #   end
    class MemoryUsageWithChildren < PollCondition
      ...
      def test
        ...
        children = File.read("/proc/#{self.pid}/task/#{self.pid}/children").split
        children.each do |pid|
          total_memory += System::Process.new(pid).memory
        end
        @timeline.push(total_memory)
